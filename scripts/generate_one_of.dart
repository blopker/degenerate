...
void main() {
  ...
  buf.writeln('/// Typed union containers for mutually exclusive values.');
  buf.writeln('///');
  ...
  buf.writeln();
  buf.writeln("import 'package:degenerate_runtime/src/json_value.dart';");
  buf.writeln("import 'package:meta/meta.dart';");
  buf.writeln();
  buf.writeln('String _oneOfError(Object? json, List<(String, Object)> errors) {');
  buf.writeln(r"  final variants = errors.map((e) => '  ${e.$1}: ${e.$2}').join('\n');");
  buf.writeln(r"  return 'No variant matched for value ($json):\n$variants';");
  buf.writeln('}');
  ...
  const outPath = 'packages/degenerate_runtime/lib/src/one_of.dart';
...
void _writeOneOf(StringBuffer buf, int n) {
  ...
  // toJson
  buf.writeln('  /// Serializes the value, including nested collections.');
  buf.writeln('  Object? toJson() => jsonValue(value);');
  buf.writeln();

...
Future<ApiResult<List<SpectrumAnalyticsQueryResponseAggregateResult>?, ResponseCommonFailure66>> spectrumAggregateAnalyticsGetCurrentAggregatedAnalytics({required SpectrumAnalyticsIdentifier zoneId, SpectrumAnalyticsAppIdParam? appId, String? coloName, RequestOptions? options, }) async  { final queryParameters = <String, String>{...apiConfig.defaultQueryParameters};
...
return execute(
  ...
  onError: (response) {
    return ResponseCommonFailure66.fromJson(jsonDecode(response.body) as Map<String, dynamic>);
  },
);
 } 
...
Future<ApiResult<SpectrumAnalyticsQueryResponseSingleResult?, ResponseCommonFailure66>> spectrumAnalyticsSummaryGetAnalyticsSummary({required SpectrumAnalyticsIdentifier zoneId, List<SpectrumAnalyticsDimensions2>? dimensions, List<String>? sort, SpectrumAnalyticsTimestamp? until, List<SpectrumAnalyticsMetrics2>? metrics, SpectrumAnalyticsFilters? filters, SpectrumAnalyticsTimestamp? since, RequestOptions? options, }) async  { final queryParameters = <String, String>{...apiConfig.defaultQueryParameters};
final queryParametersList = <ApiQueryParameter>[];
if (dimensions != null) {
for (final item in dimensions) {
  queryParametersList.add(ApiQueryParameter(name: 'dimensions', value: item.toJson()));

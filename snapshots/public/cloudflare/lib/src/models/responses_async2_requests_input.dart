//  dart format off
//  GENERATED CODE — DO NOT MODIFY BY HAND
// 
//  Generated by degenerate from cloudflare.yaml
//  OpenAPI spec version: 3.0.3

/// Responses API Input messages. Refer to OpenAI Responses API docs to learn more about supported content types
final class ResponsesAsync2RequestsInput {const ResponsesAsync2RequestsInput({this.string, this.listObject, });

factory ResponsesAsync2RequestsInput.fromJson(Object? json) { return ResponsesAsync2RequestsInput(
  string: json is String ? json : null,
  // listObject: skipped (collection type in anyOf not supported)
); }

final String? string;

final List<Object?>? listObject;

/// At least one variant must be present.
bool get isValid { return string != null || listObject != null; } 
Map<String, dynamic> toJson() { return {
  'string': ?string,
  'listObject': ?listObject,
}; } 
 }

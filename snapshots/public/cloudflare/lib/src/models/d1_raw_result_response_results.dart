...
import 'package:collection/collection.dart';import 'package:degenerate_runtime/degenerate_runtime.dart';import 'd1_raw_result_response_results_rows.dart';final class D1RawResultResponseResults {const D1RawResultResponseResults({this.columns, this.rows, });
...
factory D1RawResultResponseResults.fromJson(Map<String, dynamic> json) { return D1RawResultResponseResults(
  ...
  rows: (json['rows'] as List<dynamic>?)?.map((e) => (e as List<dynamic>).map((e) => OneOf3.parse(e, fromA: (v) => (v as num).toDouble(), canParseA: (v) => v is num, fromB: (v) => v as String, canParseB: (v) => v is String, fromC: (v) => (v as Map<String, dynamic>).map((k, v) => MapEntry(k, v)), canParseC: (v) => v is Map,)).toList()).toList(),
); }

// GENERATED CODE - DO NOT MODIFY BY HAND

import 'package:degenerate_runtime/degenerate_runtime.dart';import 'cloudflare_ips_cloudflare_ip_details_response_result.dart';import 'public_ip_ips.dart';import 'public_ip_ips_jdcloud.dart';import 'public_ip_messages2.dart';@immutable final class CloudflareIpsCloudflareIpDetailsResponse {const CloudflareIpsCloudflareIpDetailsResponse({required this.errors, required this.messages, required this.success, this.result, });

factory CloudflareIpsCloudflareIpDetailsResponse.fromJson(Map<String, dynamic> json) { return CloudflareIpsCloudflareIpDetailsResponse(
  errors: (json['errors'] as List<dynamic>).map((e) => PublicIpMessages2.fromJson(e as Map<String, dynamic>)).toList(),
  messages: (json['messages'] as List<dynamic>).map((e) => PublicIpMessages2.fromJson(e as Map<String, dynamic>)).toList(),
  success: json['success'] as bool,
  result: json['result'] != null ? OneOf2.parse(json['result'], fromA: (v) => PublicIpIps.fromJson(v as Map<String, dynamic>), fromB: (v) => PublicIpIpsJdcloud.fromJson(v as Map<String, dynamic>),) : null,
); }

final List<PublicIpMessages2> errors;

final List<PublicIpMessages2> messages;

/// Whether the API call was successful.
final bool success;

final CloudflareIpsCloudflareIpDetailsResponseResult? result;

Map<String, dynamic> toJson() { return {
  'errors': errors.map((e) => e.toJson()).toList(),
  'messages': messages.map((e) => e.toJson()).toList(),
  'success': success,
  if (result != null) 'result': result?.toJson(),
}; } 
static bool canParse(Map<String, dynamic> json) { return json.containsKey('errors') &&
      json.containsKey('messages') &&
      json.containsKey('success') && json['success'] is bool; } 
CloudflareIpsCloudflareIpDetailsResponse copyWith({List<PublicIpMessages2>? errors, List<PublicIpMessages2>? messages, bool? success, CloudflareIpsCloudflareIpDetailsResponseResult? Function()? result, }) { return CloudflareIpsCloudflareIpDetailsResponse(
  errors: errors ?? this.errors,
  messages: messages ?? this.messages,
  success: success ?? this.success,
  result: result != null ? result() : this.result,
); } 
@override bool operator ==(Object other) { return identical(this, other) ||
      other is CloudflareIpsCloudflareIpDetailsResponse &&
          listEquals(errors, other.errors) &&
          listEquals(messages, other.messages) &&
          success == other.success &&
          result == other.result; } 
@override int get hashCode { return Object.hash(Object.hashAll(errors), Object.hashAll(messages), success, result); } 
@override String toString() { return 'CloudflareIpsCloudflareIpDetailsResponse(errors: $errors, messages: $messages, success: $success, result: $result)'; } 
 }

// GENERATED CODE - DO NOT MODIFY BY HAND

import 'package:degenerate_runtime/degenerate_runtime.dart';import 'connect_account_reference.dart';/// 
@immutable final class InvoiceSettingQuoteSetting {const InvoiceSettingQuoteSetting({required this.issuer, this.daysUntilDue = const Omittable.absent(), });

factory InvoiceSettingQuoteSetting.fromJson(Map<String, dynamic> json) { return InvoiceSettingQuoteSetting(
  daysUntilDue: json.containsKey('days_until_due') ? Omittable(json['days_until_due'] != null ? (json['days_until_due'] as num).toInt() : null) : const Omittable.absent(),
  issuer: ConnectAccountReference.fromJson(json['issuer'] as Map<String, dynamic>),
); }

/// Number of days within which a customer must pay invoices generated by this quote. This value will be `null` for quotes where `collection_method=charge_automatically`.
final Omittable<int?> daysUntilDue;

final ConnectAccountReference issuer;

Map<String, dynamic> toJson() { return {
  if (daysUntilDue.isPresent) 'days_until_due': daysUntilDue.value,
  'issuer': issuer.toJson(),
}; } 
static bool canParse(Map<String, dynamic> json) { return json.containsKey('issuer'); } 
InvoiceSettingQuoteSetting copyWith({Omittable<int?>? daysUntilDue, ConnectAccountReference? issuer, }) { return InvoiceSettingQuoteSetting(
  daysUntilDue: daysUntilDue ?? this.daysUntilDue,
  issuer: issuer ?? this.issuer,
); } 
@override bool operator ==(Object other) { return identical(this, other) ||
      other is InvoiceSettingQuoteSetting &&
          daysUntilDue == other.daysUntilDue &&
          issuer == other.issuer; } 
@override int get hashCode { return Object.hash(daysUntilDue, issuer); } 
@override String toString() { return 'InvoiceSettingQuoteSetting(daysUntilDue: $daysUntilDue, issuer: $issuer)'; } 
 }

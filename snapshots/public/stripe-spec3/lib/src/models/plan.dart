...
import 'package:collection/collection.dart';import 'package:degenerate_runtime/degenerate_runtime.dart';import 'plan_product.dart';import 'plan_tier.dart';import 'plan_transform_usage.dart';/// Describes how to compute the price per period. Either `per_unit` or `tiered`. `per_unit` indicates that the fixed amount (specified in `amount`) will be charged per unit in `quantity` (for plans with `usage_type=licensed`), or per unit of total usage (for plans with `usage_type=metered`). `tiered` indicates that the unit pricing will be computed using a tiering strategy as defined using the `tiers` and `tiers_mode` attributes.
...
factory Plan.fromJson(Map<String, dynamic> json) { return Plan(
  ...
  product: json['product'] != null ? OneOf3.parse(json['product'], fromA: (v) => v as String, canParseA: (v) => v is String, fromB: (v) => Product.fromJson(v as Map<String, dynamic>), canParseB: (v) => v is Map<String, dynamic> && Product.canParse(v), fromC: (v) => DeletedProduct.fromJson(v as Map<String, dynamic>), canParseC: (v) => v is Map<String, dynamic> && DeletedProduct.canParse(v),) : null,

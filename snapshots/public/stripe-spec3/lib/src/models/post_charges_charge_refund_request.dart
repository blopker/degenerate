...
import 'package:collection/collection.dart';import 'package:degenerate_runtime/degenerate_runtime.dart';import 'post_charges_charge_refund_request_metadata.dart';/// String indicating the reason for the refund. If set, possible values are `duplicate`, `fraudulent`, and `requested_by_customer`. If you believe the charge to be fraudulent, specifying `fraudulent` as the reason will add the associated card and email to your [block lists](https://docs.stripe.com/radar/lists), and will also help us improve our fraud detection algorithms.
...
factory PostChargesChargeRefundRequest.fromJson(Map<String, dynamic> json) { return PostChargesChargeRefundRequest(
  ...
  metadata: json['metadata'] != null ? OneOf2.parse(json['metadata'], fromA: (v) => (v as Map<String, dynamic>).map((k, v) => MapEntry(k, v as String)), canParseA: (v) => v is Map, fromB: (v) => PostChargesChargeRefundRequestMetadataVariant2.fromJson(v as String), canParseB: (v) => v is String,) : null,

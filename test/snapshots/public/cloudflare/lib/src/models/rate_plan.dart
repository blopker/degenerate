//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from cloudflare.yaml
//  OpenAPI spec version: 3.0.3

import 'package:collection/collection.dart';
import 'component_value.dart';
import 'schemas_frequency.dart';

final class RatePlan {
  const RatePlan({
    this.components,
    this.currency,
    this.duration,
    this.frequency,
    this.id,
    this.name,
  });

  factory RatePlan.fromJson(Map<String, dynamic> json) {
    return RatePlan(
      components: (json['components'] as List<dynamic>?)
          ?.map((e) => ComponentValue.fromJson(e as Map<String, dynamic>))
          .toList(),
      currency: json['currency'] as String?,
      duration: json['duration'] != null
          ? (json['duration'] as num).toDouble()
          : null,
      frequency: json['frequency'] != null
          ? SchemasFrequency.fromJson(json['frequency'] as String)
          : null,
      id: json['id'] as String?,
      name: json['name'] as String?,
    );
  }

  final List<ComponentValue>? components;

  final String? currency;

  final double? duration;

  final SchemasFrequency? frequency;

  final String? id;

  final String? name;

  Map<String, dynamic> toJson() {
    return {
      if (components != null)
        'components': components?.map((e) => e.toJson()).toList(),
      'currency': ?currency,
      'duration': ?duration,
      if (frequency != null) 'frequency': frequency?.toJson(),
      'id': ?id,
      'name': ?name,
    };
  }

  static bool canParse(Map<String, dynamic> json) {
    return true;
  }

  RatePlan copyWith({
    List<ComponentValue> Function()? components,
    String Function()? currency,
    double Function()? duration,
    SchemasFrequency Function()? frequency,
    String Function()? id,
    String Function()? name,
  }) {
    return RatePlan(
      components: components != null ? components() : this.components,
      currency: currency != null ? currency() : this.currency,
      duration: duration != null ? duration() : this.duration,
      frequency: frequency != null ? frequency() : this.frequency,
      id: id != null ? id() : this.id,
      name: name != null ? name() : this.name,
    );
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is RatePlan &&
            const ListEquality<dynamic>().equals(
              components,
              other.components,
            ) &&
            currency == other.currency &&
            duration == other.duration &&
            frequency == other.frequency &&
            id == other.id &&
            name == other.name;
  }

  @override
  int get hashCode {
    return Object.hash(
      Object.hashAll(components ?? const []),
      currency,
      duration,
      frequency,
      id,
      name,
    );
  }

  @override
  String toString() {
    return 'RatePlan(components: $components, currency: $currency, duration: $duration, frequency: $frequency, id: $id, name: $name)';
  }
}

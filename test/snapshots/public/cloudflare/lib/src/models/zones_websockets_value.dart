//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from cloudflare.yaml
//  OpenAPI spec version: 3.0.3

/// Value of the zone setting.
final class ZonesWebsocketsValue {
  const ZonesWebsocketsValue._(this.value);

  factory ZonesWebsocketsValue.fromJson(String json) {
    return switch (json) {
      'off' => off,
      'on' => $on,
      _ => ZonesWebsocketsValue._(json),
    };
  }

  static const ZonesWebsocketsValue off = ZonesWebsocketsValue._('off');

  static const ZonesWebsocketsValue $on = ZonesWebsocketsValue._('on');

  static const List<ZonesWebsocketsValue> values = [off, $on];

  final String value;

  String toJson() {
    return value;
  }

  /// Whether this value is unknown (not defined in the OpenAPI spec).
  bool get isUnknown {
    return !values.contains(this);
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is ZonesWebsocketsValue && other.value == value;
  }

  @override
  int get hashCode {
    return value.hashCode;
  }

  @override
  String toString() {
    return 'ZonesWebsocketsValue($value)';
  }
}

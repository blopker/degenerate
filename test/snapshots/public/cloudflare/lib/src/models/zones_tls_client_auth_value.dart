//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from cloudflare.yaml
//  OpenAPI spec version: 3.0.3

/// value of the zone setting.
final class ZonesTlsClientAuthValue {
  const ZonesTlsClientAuthValue._(this.value);

  factory ZonesTlsClientAuthValue.fromJson(String json) {
    return switch (json) {
      'on' => $on,
      'off' => off,
      _ => ZonesTlsClientAuthValue._(json),
    };
  }

  static const ZonesTlsClientAuthValue $on = ZonesTlsClientAuthValue._('on');

  static const ZonesTlsClientAuthValue off = ZonesTlsClientAuthValue._('off');

  static const List<ZonesTlsClientAuthValue> values = [$on, off];

  final String value;

  String toJson() {
    return value;
  }

  /// Whether this value is unknown (not defined in the OpenAPI spec).
  bool get isUnknown {
    return !values.contains(this);
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is ZonesTlsClientAuthValue && other.value == value;
  }

  @override
  int get hashCode {
    return value.hashCode;
  }

  @override
  String toString() {
    return 'ZonesTlsClientAuthValue($value)';
  }
}

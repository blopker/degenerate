//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from cloudflare.yaml
//  OpenAPI spec version: 3.0.3

/// Bypass cache and fetch resources from the origin server if a regular
/// expression matches against a cookie name present in the request.
///
final class ZonesBypassCacheOnCookieId {
  const ZonesBypassCacheOnCookieId._(this.value);

  factory ZonesBypassCacheOnCookieId.fromJson(String json) {
    return switch (json) {
      'bypass_cache_on_cookie' => bypassCacheOnCookie,
      _ => ZonesBypassCacheOnCookieId._(json),
    };
  }

  static const ZonesBypassCacheOnCookieId bypassCacheOnCookie =
      ZonesBypassCacheOnCookieId._('bypass_cache_on_cookie');

  static const List<ZonesBypassCacheOnCookieId> values = [bypassCacheOnCookie];

  final String value;

  String toJson() {
    return value;
  }

  /// Whether this value is unknown (not defined in the OpenAPI spec).
  bool get isUnknown {
    return !values.contains(this);
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is ZonesBypassCacheOnCookieId && other.value == value;
  }

  @override
  int get hashCode {
    return value.hashCode;
  }

  @override
  String toString() {
    return 'ZonesBypassCacheOnCookieId($value)';
  }
}

//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from github-rest-3.1.yaml
//  OpenAPI spec version: 3.1.0

/// State of the project; either 'open' or 'closed'
final class WebhooksProjectState {
  const WebhooksProjectState._(this.value);

  factory WebhooksProjectState.fromJson(String json) {
    return switch (json) {
      'open' => open,
      'closed' => closed,
      _ => WebhooksProjectState._(json),
    };
  }

  static const WebhooksProjectState open = WebhooksProjectState._('open');

  static const WebhooksProjectState closed = WebhooksProjectState._('closed');

  static const List<WebhooksProjectState> values = [open, closed];

  final String value;

  String toJson() {
    return value;
  }

  /// Whether this value is unknown (not defined in the OpenAPI spec).
  bool get isUnknown {
    return !values.contains(this);
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is WebhooksProjectState && other.value == value;
  }

  @override
  int get hashCode {
    return value.hashCode;
  }

  @override
  String toString() {
    return 'WebhooksProjectState($value)';
  }
}

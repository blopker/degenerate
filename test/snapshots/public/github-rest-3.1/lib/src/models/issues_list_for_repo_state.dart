//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from github-rest-3.1.yaml
//  OpenAPI spec version: 3.1.0

final class IssuesListForRepoState {
  const IssuesListForRepoState._(this.value);

  factory IssuesListForRepoState.fromJson(String json) {
    return switch (json) {
      'open' => open,
      'closed' => closed,
      'all' => all,
      _ => IssuesListForRepoState._(json),
    };
  }

  static const IssuesListForRepoState open = IssuesListForRepoState._('open');

  static const IssuesListForRepoState closed = IssuesListForRepoState._(
    'closed',
  );

  static const IssuesListForRepoState all = IssuesListForRepoState._('all');

  static const List<IssuesListForRepoState> values = [open, closed, all];

  final String value;

  String toJson() {
    return value;
  }

  /// Whether this value is unknown (not defined in the OpenAPI spec).
  bool get isUnknown {
    return !values.contains(this);
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is IssuesListForRepoState && other.value == value;
  }

  @override
  int get hashCode {
    return value.hashCode;
  }

  @override
  String toString() {
    return 'IssuesListForRepoState($value)';
  }
}

//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from github-rest-3.1.yaml
//  OpenAPI spec version: 3.1.0

/// The type of pricing for the budget
final class BudgetBudgetType {
  const BudgetBudgetType._(this.value);

  factory BudgetBudgetType.fromJson(String json) {
    return switch (json) {
      'SkuPricing' => skuPricing,
      'ProductPricing' => productPricing,
      _ => BudgetBudgetType._(json),
    };
  }

  static const BudgetBudgetType skuPricing = BudgetBudgetType._('SkuPricing');

  static const BudgetBudgetType productPricing = BudgetBudgetType._(
    'ProductPricing',
  );

  static const List<BudgetBudgetType> values = [skuPricing, productPricing];

  final String value;

  String toJson() {
    return value;
  }

  /// Whether this value is unknown (not defined in the OpenAPI spec).
  bool get isUnknown {
    return !values.contains(this);
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is BudgetBudgetType && other.value == value;
  }

  @override
  int get hashCode {
    return value.hashCode;
  }

  @override
  String toString() {
    return 'BudgetBudgetType($value)';
  }
}

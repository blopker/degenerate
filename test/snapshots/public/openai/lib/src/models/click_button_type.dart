//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from openai.yml
//  OpenAPI spec version: 3.1.0

final class ClickButtonType {
  const ClickButtonType._(this.value);

  factory ClickButtonType.fromJson(String json) {
    return switch (json) {
      'left' => left,
      'right' => right,
      'wheel' => wheel,
      'back' => back,
      'forward' => forward,
      _ => ClickButtonType._(json),
    };
  }

  static const ClickButtonType left = ClickButtonType._('left');

  static const ClickButtonType right = ClickButtonType._('right');

  static const ClickButtonType wheel = ClickButtonType._('wheel');

  static const ClickButtonType back = ClickButtonType._('back');

  static const ClickButtonType forward = ClickButtonType._('forward');

  static const List<ClickButtonType> values = [
    left,
    right,
    wheel,
    back,
    forward,
  ];

  final String value;

  String toJson() {
    return value;
  }

  /// Whether this value is unknown (not defined in the OpenAPI spec).
  bool get isUnknown {
    return !values.contains(this);
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is ClickButtonType && other.value == value;
  }

  @override
  int get hashCode {
    return value.hashCode;
  }

  @override
  String toString() {
    return 'ClickButtonType($value)';
  }
}

//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from openai.yml
//  OpenAPI spec version: 3.1.0

import 'package:collection/collection.dart';
import 'batch_errors_data.dart';

final class BatchErrors {
  const BatchErrors({this.object, this.data});

  factory BatchErrors.fromJson(Map<String, dynamic> json) {
    return BatchErrors(
      object: json['object'] as String?,
      data: (json['data'] as List<dynamic>?)
          ?.map((e) => BatchErrorsData.fromJson(e as Map<String, dynamic>))
          .toList(),
    );
  }

  /// The object type, which is always `list`.
  final String? object;

  final List<BatchErrorsData>? data;

  Map<String, dynamic> toJson() {
    return {
      'object': ?object,
      if (data != null) 'data': data?.map((e) => e.toJson()).toList(),
    };
  }

  static bool canParse(Map<String, dynamic> json) {
    return true;
  }

  BatchErrors copyWith({
    String Function()? object,
    List<BatchErrorsData> Function()? data,
  }) {
    return BatchErrors(
      object: object != null ? object() : this.object,
      data: data != null ? data() : this.data,
    );
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is BatchErrors &&
            object == other.object &&
            const ListEquality<dynamic>().equals(data, other.data);
  }

  @override
  int get hashCode {
    return Object.hash(object, Object.hashAll(data ?? const []));
  }

  @override
  String toString() {
    return 'BatchErrors(object: $object, data: $data)';
  }
}

//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from openai.yml
//  OpenAPI spec version: 3.1.0

import 'package:collection/collection.dart';
import 'create_embedding_response_object.dart';
import 'create_embedding_response_usage.dart';
import 'embedding.dart';

final class CreateEmbeddingResponse {
  const CreateEmbeddingResponse({
    required this.data,
    required this.model,
    required this.object,
    required this.usage,
  });

  factory CreateEmbeddingResponse.fromJson(Map<String, dynamic> json) {
    return CreateEmbeddingResponse(
      data: (json['data'] as List<dynamic>)
          .map((e) => Embedding.fromJson(e as Map<String, dynamic>))
          .toList(),
      model: json['model'] as String,
      object: CreateEmbeddingResponseObject.fromJson(json['object'] as String),
      usage: CreateEmbeddingResponseUsage.fromJson(
        json['usage'] as Map<String, dynamic>,
      ),
    );
  }

  /// The list of embeddings generated by the model.
  final List<Embedding> data;

  /// The name of the model used to generate the embedding.
  final String model;

  /// The object type, which is always "list".
  final CreateEmbeddingResponseObject object;

  /// The usage information for the request.
  final CreateEmbeddingResponseUsage usage;

  Map<String, dynamic> toJson() {
    return {
      'data': data.map((e) => e.toJson()).toList(),
      'model': model,
      'object': object.toJson(),
      'usage': usage.toJson(),
    };
  }

  static bool canParse(Map<String, dynamic> json) {
    return json.containsKey('data') &&
        json.containsKey('model') &&
        json['model'] is String &&
        json.containsKey('object') &&
        json.containsKey('usage');
  }

  CreateEmbeddingResponse copyWith({
    List<Embedding>? data,
    String? model,
    CreateEmbeddingResponseObject? object,
    CreateEmbeddingResponseUsage? usage,
  }) {
    return CreateEmbeddingResponse(
      data: data ?? this.data,
      model: model ?? this.model,
      object: object ?? this.object,
      usage: usage ?? this.usage,
    );
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is CreateEmbeddingResponse &&
            const ListEquality<dynamic>().equals(data, other.data) &&
            model == other.model &&
            object == other.object &&
            usage == other.usage;
  }

  @override
  int get hashCode {
    return Object.hash(Object.hashAll(data), model, object, usage);
  }

  @override
  String toString() {
    return 'CreateEmbeddingResponse(data: $data, model: $model, object: $object, usage: $usage)';
  }
}

//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from openai.yml
//  OpenAPI spec version: 3.1.0

/// Defines an inline skill for this request.
final class InlineSkillParamType {
  const InlineSkillParamType._(this.value);

  factory InlineSkillParamType.fromJson(String json) {
    return switch (json) {
      'inline' => inline,
      _ => InlineSkillParamType._(json),
    };
  }

  static const InlineSkillParamType inline = InlineSkillParamType._('inline');

  static const List<InlineSkillParamType> values = [inline];

  final String value;

  String toJson() {
    return value;
  }

  /// Whether this value is unknown (not defined in the OpenAPI spec).
  bool get isUnknown {
    return !values.contains(this);
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is InlineSkillParamType && other.value == value;
  }

  @override
  int get hashCode {
    return value.hashCode;
  }

  @override
  String toString() {
    return 'InlineSkillParamType($value)';
  }
}

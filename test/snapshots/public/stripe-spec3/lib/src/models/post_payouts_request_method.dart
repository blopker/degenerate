//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from stripe-spec3.yaml
//  OpenAPI spec version: 3.0.0

/// The method used to send this payout, which is `standard` or `instant`. We support `instant` for payouts to debit cards and bank accounts in certain countries. Learn more about [bank support for Instant Payouts](https://stripe.com/docs/payouts/instant-payouts-banks).
final class PostPayoutsRequestMethod {
  const PostPayoutsRequestMethod._(this.value);

  factory PostPayoutsRequestMethod.fromJson(String json) {
    return switch (json) {
      'instant' => instant,
      'standard' => standard,
      _ => PostPayoutsRequestMethod._(json),
    };
  }

  static const PostPayoutsRequestMethod instant = PostPayoutsRequestMethod._(
    'instant',
  );

  static const PostPayoutsRequestMethod standard = PostPayoutsRequestMethod._(
    'standard',
  );

  static const List<PostPayoutsRequestMethod> values = [instant, standard];

  final String value;

  String toJson() {
    return value;
  }

  /// Whether this value is unknown (not defined in the OpenAPI spec).
  bool get isUnknown {
    return !values.contains(this);
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is PostPayoutsRequestMethod && other.value == value;
  }

  @override
  int get hashCode {
    return value.hashCode;
  }

  @override
  String toString() {
    return 'PostPayoutsRequestMethod($value)';
  }
}

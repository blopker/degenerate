//  GENERATED CODE — DO NOT MODIFY BY HAND
//
//  Generated by degenerate from stripe-spec3.yaml
//  OpenAPI spec version: 3.0.0

/// Reason for the refund, which is either user-provided (`duplicate`, `fraudulent`, or `requested_by_customer`) or generated by Stripe internally (`expired_uncaptured_charge`).
final class RefundReason {
  const RefundReason._(this.value);

  factory RefundReason.fromJson(String json) {
    return switch (json) {
      'duplicate' => duplicate,
      'expired_uncaptured_charge' => expiredUncapturedCharge,
      'fraudulent' => fraudulent,
      'requested_by_customer' => requestedByCustomer,
      _ => RefundReason._(json),
    };
  }

  static const RefundReason duplicate = RefundReason._('duplicate');

  static const RefundReason expiredUncapturedCharge = RefundReason._(
    'expired_uncaptured_charge',
  );

  static const RefundReason fraudulent = RefundReason._('fraudulent');

  static const RefundReason requestedByCustomer = RefundReason._(
    'requested_by_customer',
  );

  static const List<RefundReason> values = [
    duplicate,
    expiredUncapturedCharge,
    fraudulent,
    requestedByCustomer,
  ];

  final String value;

  String toJson() {
    return value;
  }

  /// Whether this value is unknown (not defined in the OpenAPI spec).
  bool get isUnknown {
    return !values.contains(this);
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is RefundReason && other.value == value;
  }

  @override
  int get hashCode {
    return value.hashCode;
  }

  @override
  String toString() {
    return 'RefundReason($value)';
  }
}
